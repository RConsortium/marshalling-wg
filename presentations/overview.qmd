...
-   Serialization is for example used to:
    ...
    2.  Communicating an object to another R process for *parallelization*

# Shortcomings of Serialization in R
...
-   This e.g. causes problems with serializing many `R6` objects, whose methods all point to the same function within an R Session
...
-   "Marshalling is the process of transforming the memory representation of
    an object into a data format suitable for storage or transmission,
    especially between different runtimes." (Wikipedia)
...
-   `serialize()` and `unserialize()` have a `refhook` argument that allows to customize handling of reference objects (such as external pointers)
...
-   One disadvantage of first marshalling an R object and then serializing it is that in many cases, we could do away with the additional allocation of the `marshal()` call

-   This might be able to yield some considerable efficiency gains
